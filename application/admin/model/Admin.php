...
class Admin extends Model
{
    public function info(){
        $admin=Admin::getByName(session('name'));
        if($admin){
            return $admin;
        }else{
            return null; //用户不存在
    ...
    if($this->save($adminData)){
        ...
        db('hatch_auth_group_access')->insert($groupAccess);
        return true;
    }else{
        return false;
    }
   ...
   public function saveadmin($data,$admins){
        ...
        db('hatch_auth_group_access')->where(array('uid'=>$data['id']))->update(['group_id'=>$data['group_id']]);
